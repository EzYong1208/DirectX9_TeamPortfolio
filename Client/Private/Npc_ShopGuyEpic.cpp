#include "stdafx.h"
#include "..\Public\Npc_ShopGuyEpic.h"
#include "Texture.h"
#include "Renderer.h"
#include "Transform.h"
...
CNpc_ShopGuyEpic::CNpc_ShopGuyEpic(LPDIRECT3DDEVICE9 _pGraphic_Device)
	: CGameObject(_pGraphic_Device)
{
}

CNpc_ShopGuyEpic::CNpc_ShopGuyEpic(const CNpc_ShopGuyEpic & rhs)
	: CGameObject(rhs)
{
}

HRESULT CNpc_ShopGuyEpic::NativeConstruct_Prototype()
{
...
HRESULT CNpc_ShopGuyEpic::NativeConstruct(void * _pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, _float3(51.6f, 0.5f, 36.9f));
	m_pTransformCom->Scaled(_float3(1.f, 1.f, 1.f));

	return S_OK;
}

_int CNpc_ShopGuyEpic::Tick(_float _fTimeDelta)
{
	if (0 >__super::Tick(_fTimeDelta))
		return -1;

	m_fFrame += 13.0f * _fTimeDelta * 0.3f;
	...
	if (true == Collision_Check_Player())
	{
		...
		if (pGameInstance->Key_Down('Z'))
		{
			if (0 == m_iCount)
			{
				...
				pGameInstance->Get_UI(LEVEL_GAMEPLAY, TEXT("UI_Epic_Shop"))->Change_UI_State();
...
_int CNpc_ShopGuyEpic::LateTick(_float _fTimeDelta)
{
...
HRESULT CNpc_ShopGuyEpic::Render()
{
	__super::Render();
...
HRESULT CNpc_ShopGuyEpic::SetUp_Components()
{
	//	For.Com_Renderer
	...
	if (FAILED(Add_Component(
		LEVEL_GAMEPLAY,
		L"Prototype_Component_Texture_Npc_ShopGuyEpic",
		L"Com_Texture_Npc",
...
_bool CNpc_ShopGuyEpic::Collision_Check_Player()
{
	_bool bResult = false;
...
HRESULT CNpc_ShopGuyEpic::Save_Data(HANDLE _hFile)
{
...
HRESULT CNpc_ShopGuyEpic::Load_Data(HANDLE _hFile)
{
	if (_hFile == INVALID_HANDLE_VALUE)
	{
		MSGBOX("INVALID_HANDLE_VALUE");
...
CNpc_ShopGuyEpic * CNpc_ShopGuyEpic::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CNpc_ShopGuyEpic* pInstance = new CNpc_ShopGuyEpic(_pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CNpc_ShopGuyEpic")
...
CGameObject * CNpc_ShopGuyEpic::Clone(void * _pArg)
{
	CNpc_ShopGuyEpic* pInstance = new CNpc_ShopGuyEpic(*this);
