...
CNpc_Ghost::CNpc_Ghost(LPDIRECT3DDEVICE9 _pGraphic_Device)
	: CGameObject(_pGraphic_Device)
{
}

CNpc_Ghost::CNpc_Ghost(const CNpc_Ghost & rhs)
	: CGameObject(rhs)
{
}

HRESULT CNpc_Ghost::NativeConstruct_Prototype()
{
...
HRESULT CNpc_Ghost::NativeConstruct(void * _pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, _float3(7.5f, 0.5f, 46.5f));
	m_pTransformCom->Scaled(_float3(1.f, 1.f, 1.f));

	return S_OK;
}

_int CNpc_Ghost::Tick(_float _fTimeDelta)
{
	if (0 >__super::Tick(_fTimeDelta))
		return -1;

	m_fFrame += 6.0f * _fTimeDelta * 0.3f;
...
_int CNpc_Ghost::LateTick(_float _fTimeDelta)
{
...
HRESULT CNpc_Ghost::SetUp_Components()
{
	//	For.Com_Renderer
	...
	if (FAILED(Add_Component(
		LEVEL_GAMEPLAY,
		L"Prototype_Component_Texture_Npc_Ghost",
		L"Com_Texture_Npc",
...
_bool CNpc_Ghost::Collision_Check_Player()
{
	_bool bResult = false;
...
HRESULT CNpc_Ghost::Save_Data(HANDLE _hFile)
{
...
HRESULT CNpc_Ghost::Load_Data(HANDLE _hFile)
{
	if (_hFile == INVALID_HANDLE_VALUE)
	{
		MSGBOX("INVALID_HANDLE_VALUE");
...
CNpc_Ghost * CNpc_Ghost::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CNpc_Ghost* pInstance = new CNpc_Ghost(_pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CNpc_Ghost")
...
CGameObject * CNpc_Ghost::Clone(void * _pArg)
{
	CNpc_Ghost* pInstance = new CNpc_Ghost(*this);
